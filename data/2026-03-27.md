...
TL;DR: 研究发现抑郁症自动检测模型在医患对话分析中存在系统性偏差：模型主要利用访谈者的固定提示词而非患者语言进行预测，导致性能虚高


<details>
  <summary>Details</summary>
Motivation: 尽管基于医患对话的自动抑郁症检测取得进展，但现有模型的可解释性有限，无法揭示预测的真正驱动因素。研究者怀疑模型可能利用了数据集中的系统性偏差而非真实的语言特征

Method: 分析三个公开数据集（ANDROIDS、DAIC-WOZ、E-DAIC），通过对比模型在完整对话与仅患者话语上的表现，识别访谈者提示词带来的偏差。研究考察了不同模型架构在不同数据条件下的分类性能

Result: 模型在完整对话中主要利用访谈者的固定提示词和位置信息来区分抑郁与控制组，即使不使用患者语言也能获得高分类分数。限制模型仅使用患者话语后，决策证据分布更广泛，反映了真实的语言线索。半结构化访谈协议虽然确保一致性，但包含访谈者提示会因脚本伪影而虚增性能

Conclusion: 研究揭示了跨数据集、架构无关的系统性偏差，强调需要通过时间和说话者定位决策证据的分析方法，确保模型真正从患者语言中学习，而非利用数据集伪影
...
TL;DR: 通过微调小型开源大语言模型，显著提升了系统综述中标题和摘要筛选的自动化性能


<details>
  <summary>Details</summary>
Motivation: 传统系统综述需要大量人工筛选标题和摘要，耗时耗力。现有研究尝试使用大语言模型提高效率，但仅通过提示工程效果不稳定，需要更有效的方法。

Method: 在包含8500多个人工标注标题和摘要的系统综述数据集上，对1.2亿参数的开源大语言模型进行微调，专门用于研究筛选任务。

Result: 微调后模型性能显著提升：加权F1分数比基础模型提高80.79%；在8277项研究的完整数据集上，与人工编码员的一致性达86.40%，真阳性率91.18%，真阴性率86.38%，多次推理运行结果完全一致。

Conclusion: 微调大语言模型在大型系统综述的标题和摘要筛选中具有良好应用前景，能显著提高自动化筛选的准确性和一致性。
...
TL;DR: 该研究通过微调LLaMA 3.1-8B模型，探索了针对芬兰语医学转录的领域对齐自然语言处理方法，结果显示在语义相似度方面表现良好。


<details>
  <summary>Details</summary>
Motivation: 临床文档对患者安全、诊断和护理连续性至关重要，但电子健康记录的管理负担是医生职业倦怠的重要因素。对于芬兰语等低资源语言，这一问题尤为突出，需要有效的医学转录解决方案。

Method: 使用Metropolia应用科学大学学生模拟临床对话的小型验证语料库，对LLaMA 3.1-8B模型进行微调。采用受控预处理和优化方法，通过七折交叉验证评估微调效果。
...
<details>
  <summary>Details</summary>
Motivation: 时体（aspect）对于完整捕捉句子意义至关重要，它描述事件的内部时间结构。然而，在语义表示框架中，时体标注仍然稀疏，这阻碍了手动标注和自动预测系统的发展。

Method: 创建了英语句子数据集，在AMR图上标注UMR时体标签；制定了标注方案和指南，按照UMR时体格对事件性谓词进行标注；采用多步骤裁决流程确保标注一致性和质量；使用三种建模方法进行基线实验。

Result: 建立了首个UMR时体标注数据集；通过基线实验为自动UMR时体预测建立了初始基准；为更广泛地将时体整合到语义表示中提供了基础。

Conclusion: 该数据集填补了语义表示中时体标注的空白，为未来自动化时体预测提供了实用资源，并建立了初步的基准性能。
...
TL;DR: 在葡萄牙语持续预训练中，通过文档重写生成合成数据对模型性能的提升效果取决于源数据质量，高质量源数据重写带来显著增益，低质量源数据重写效果有限，且这种交互效应随模型规模变化。


<details>
  <summary>Details</summary>
Motivation: 现有合成数据生成研究主要关注英语，且未系统控制源数据质量。本研究旨在探究在葡萄牙语持续预训练中，合成重写如何与源数据质量相互作用，验证合成数据是否可作为数据筛选的替代方案。

Method: 使用ClassiCC-PT葡萄牙语语料库（标注STEM和教育质量分数），构建两个不同质量水平的10B词元子集，使用7B指令调优模型将每个子集重写为四种风格，每个条件生成约40B词元合成数据。在两个英语中心基础模型（1.1B和7B参数）上训练，并在PoETa V2（44任务葡萄牙语基准）上评估。

Result: 在7B规模下，重写高质量数据比未修改的相同数据带来+3.4 NPM增益，而重写低质量数据仅提供+0.5 NPM增益。在1.1B规模下，这种交互效应较弱，未修改的低质量数据与重写的高质量数据表现相当。合成重写主要作为质量乘数而非数据筛选的替代方案，且这种效应具有规模依赖性。

Conclusion: 合成数据重写主要作为质量乘数而非数据筛选的替代方案，其效果强烈依赖于源数据质量和模型规模。高质量源数据重写能显著提升模型性能，而低质量源数据重写效果有限，表明数据质量筛选仍然是关键步骤。
...
Method: ARRoL训练一个轻量级质量头来预测部分rollout的成功概率，用于在生成过程中做出早期剪枝决策。系统设计将剪枝集成到推理引擎内部，重新批处理剩余rollout以计算对数概率和策略更新。质量头还可用于测试时缩放中加权候选以提高推理准确性。

Result: 在Qwen-3和LLaMA-3.2模型（1B-8B）上的GRPO和DAPO实验中，ARRoL将平均准确率提高了+2.30到+2.99，同时实现了高达1.7倍的训练加速，在测试时缩放中获得了高达+8.33的额外平均准确率增益。

Conclusion: ARRoL通过在线剪枝rollout并引导剩余rollout更平衡，有效解决了RLVR的计算成本和稀疏奖励问题，显著提高了训练效率和模型性能，同时增强了测试时的推理准确性。
...
TL;DR: LogSigma系统在SemEval-2026 Task 3中采用学习型同方差不确定性方法，通过自动学习任务特定的对数方差参数来平衡不同语言的效价-唤醒度预测难度，在五个数据集上获得第一名。


<details>
  <summary>Details</summary>
Motivation: 传统ABSA预测离散情感标签，而DimABSA需要预测1-9尺度的连续效价和唤醒度分数。核心挑战在于不同语言和领域中效价和唤醒度的预测难度存在差异，需要一种能够自动平衡不同回归目标的方法。

Method: 使用学习型同方差不确定性方法，模型学习任务特定的对数方差参数来自动平衡训练过程中的每个回归目标。结合语言特定编码器和多种子集成技术，构建LogSigma系统。

Result: LogSigma在SemEval-2026 Task 3的两个赛道中，在五个数据集上均获得第一名。学习到的方差权重在不同语言间差异显著，从德语的0.66倍到英语的2.18倍，表明最优任务平衡是语言依赖的，无法先验确定。

Conclusion: 学习型同方差不确定性方法能够有效处理DimABSA中不同语言效价-唤醒度预测难度的差异，通过自动学习任务平衡参数实现跨语言最优性能，证明了语言特定任务平衡的必要性。
...
TL;DR: 提出解码约束束搜索方法，以低成本计算近似逐字提取风险的确定性下界，解决传统方法无法有效量化近似逐字记忆的问题。


<details>
  <summary>Details</summary>
Motivation: 传统贪婪解码提取方法无法捕捉序列间的提取风险差异，而概率提取方法仅适用于逐字记忆，忽略了具有相似隐私和版权风险的近似逐字实例。量化近似逐字提取风险计算成本高昂，需要大量蒙特卡洛采样。

Method: 引入解码约束束搜索方法，该方法能够以约20个蒙特卡洛样本的成本，为近似逐字提取风险提供确定性下界估计。

Result: 该方法揭示了传统逐字方法无法发现的信息：更多可提取序列、更大的每序列提取质量，以及近似逐字提取风险在不同模型规模和文本类型中的表现模式。

Conclusion: 解码约束束搜索为高效量化近似逐字提取风险提供了实用解决方案，能够更全面地评估大语言模型的记忆和隐私风险。
...
TL;DR: 该论文研究机器翻译和质量评估在专业领域的适应问题，通过数据选择、分阶段训练、词汇对齐和上下文学习等方法提升领域适应性。


<details>
  <summary>Details</summary>
Motivation: 机器翻译和质量评估系统在通用领域表现良好，但在专业领域存在性能下降问题。需要开发有效的方法来适应特定领域，提高系统在专业场景下的可靠性和性能。

Method: 1. 基于相似性的数据选择方法用于机器翻译；2. 结合领域适应和轻量级数据增强的分阶段质量评估训练流程；3. 研究子词分词和词汇在微调中的作用；4. 提出质量评估引导的上下文学习方法，用于大型语言模型。

Result: 1. 小型针对性领域子集优于大型通用数据集，以更低计算成本达到强翻译质量；2. 分阶段训练方法在跨领域、语言和资源设置中提升性能；3. 对齐的分词-词汇设置带来稳定训练和更好翻译质量；4. 质量评估引导的上下文学习方法优于标准检索方法，支持无参考设置。

Conclusion: 领域适应依赖于数据选择、表示和高效适应策略。该论文提供了构建在领域特定设置中可靠运行的机器翻译和质量评估系统的方法，展示了数据为中心的方法在领域适应中的重要性。
...
TL;DR: MoFA是一个基于LLM推理的模型驱动特征选择框架，通过结合语义和定量特征信息进行顺序推理，在真实工业应用中提升模型精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统特征选择方法依赖标注数据和统计启发式方法，难以应用于标注数据有限且需要满足多种操作约束的生产环境。需要一种能够利用特征语义信息和定量指标进行智能特征选择的解决方案。

Method: 提出Model Feature Agent (MoFA)框架，将特征定义、重要性分数、相关性、元数据（如特征组或类型）整合到结构化提示中，通过可解释的、约束感知的推理进行顺序特征选择。

Result: 在三个真实工业应用中验证：1) 真实兴趣和时间价值预测：提高精度同时降低特征组复杂度；2) 价值模型增强：发现高阶交互项，在线实验中带来显著参与度提升；3) 通知行为预测：选择紧凑的高价值特征子集，同时提高模型精度和推理效率。

Conclusion: MoFA证明了基于LLM推理的特征选择方法在实际生产系统中的实用性和有效性，能够处理标注数据有限和多种操作约束的工业场景。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的快速发展模糊了人类写作与AI生成文本的边界，带来了错误信息传播、作者身份模糊和知识产权威胁等社会风险。现有免训练方法通常假设令牌贡献均匀，在短序列或局部令牌修改下鲁棒性不足。

Method: 提出Exons-Detect方法，采用外显子感知令牌重加权视角：1）在双模型设置下通过测量隐藏状态差异来识别和放大信息性外显子令牌；2）从重要性加权的令牌序列计算可解释的翻译分数。

Result: 实验评估表明，Exons-Detect实现了最先进的检测性能，对对抗攻击和不同输入长度表现出强鲁棒性。在DetectRL数据集上，相对于先前最强基线实现了2.2%的平均AUROC相对提升。
...
TL;DR: 研究发现：相同语义内容的系统提示在不同语言中呈现相反交互拓扑（英语合作vs西班牙语竞争），这由社交语域（特别是祈使语气）调节，通过陈述性改写可减少81%跨语言差异


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解大语言模型在多语言环境中如何处理指令，特别是相同语义内容在不同语言中为何会产生相反的交互拓扑结构（合作vs竞争），这揭示了模型将指令视为社会行为而非技术规范的本质

Method: 方法包括：1）在4种语言和4个模型上进行指令级消融实验；2）将生产系统提示分解为56个块，创建22个手动编写的探针；3）通过陈述性改写（将祈使句改为陈述句）减少跨语言差异；4）使用置换检验进行统计分析

Result: 主要结果：1）祈使语气在不同语言社区承载不同的强制力；2）陈述性改写单个指令块可减少81%跨语言差异（p=0.029）；3）改写11个祈使块中的3个可将西班牙语指令拓扑从竞争性转变为合作性；4）未改写块也出现溢出效应

Conclusion: 结论：模型将指令处理为社会行为而非技术规范，指令的强制力是语言依赖的。这暗示：1）推理时语域调节指令遵循；2）训练时可能同样如此；3）用祈使语气编写的宪法AI原则可能产生语言依赖的对齐
...
TL;DR: 研究发现LLM口头表达的置信度与其实际准确性脱节，校准信号与口头置信度信号线性正交，推理过程会污染口头置信度，提出两阶段自适应引导方法改善校准对齐


<details>
  <summary>Details</summary>
Motivation: 大型语言模型口头表达的置信度分数往往与其实际准确性脱节，但控制这种行为的几何关系仍不清楚。研究旨在通过机制可解释性分析理解口头置信度的编码机制

Method: 使用线性探针和对比激活加法引导技术分析三个开源模型和四个数据集，发现校准信号与口头置信度信号线性正交但相互独立。提出"推理污染效应"概念，并开发两阶段自适应引导管道，读取模型内部准确性估计并引导口头输出与之匹配

Result: 校准和口头置信度信号线性编码但相互正交，这一发现在所有评估模型和数据集上一致。推理过程会破坏口头置信度方向，加剧校准失准。提出的两阶段自适应引导方法显著改善了所有评估模型的校准对齐

Conclusion: LLM口头置信度与校准信号在表示空间中正交分离，推理过程会污染口头置信度。通过读取内部准确性估计并引导口头输出，可以有效改善校准对齐，为LLM置信度校准提供了新的机制理解和实用方法
...
TL;DR: 该论文提出了基于Type-2信号检测理论的LLM置信度评估框架，将模型的知识能力（Type-1敏感性）与元认知能力（Type-2元认知敏感性）分离，使用meta-d'和M-ratio指标，在四个LLM上进行大规模事实QA测试，揭示了传统校准指标的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统LLM置信度评估使用校准指标（ECE、Brier分数）混淆了两种不同能力：模型知道多少（Type-1敏感性）和模型知道自己知道什么（Type-2元认知敏感性）。需要分离这两种能力以更准确地评估模型的元认知能力。

Method: 引入基于Type-2信号检测理论的评估框架，使用meta-d'（元认知敏感性）和M-ratio（元认知效率比）指标。在四个LLM（Llama-3-8B-Instruct、Mistral-7B-Instruct-v0.3、Llama-3-8B-Base、Gemma-2-9B-Instruct）上进行了224,000个事实QA试验，分析元认知效率的模型间差异、领域特异性、温度对置信策略的影响，以及不同指标间的对比。

Result: 1. 元认知效率在不同模型间差异显著，即使Type-1敏感性相似；Mistral具有最高的d'但最低的M-ratio。2. 元认知效率具有领域特异性，不同模型在不同领域表现最弱，这在聚合指标中不可见。3. 温度操作改变Type-2标准而meta-d'保持稳定（两个模型），分离了置信策略与元认知能力。4. AUROC_2和M-ratio产生完全相反的模型排名，表明这些指标回答根本不同的评估问题。

Conclusion: meta-d'框架揭示了哪些模型"知道自己不知道什么"与哪些仅仅因为标准设置而显得校准良好之间的区别，这对模型选择、部署和人机协作有直接意义。传统校准指标无法区分元认知能力与置信策略，需要更精细的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 探索自动提示优化是否能替代专家提示工程在语言任务中的应用，填补该领域系统比较研究的空白。

Method: 使用五种模型配置，系统比较三种提示方法：手工设计的零样本专家提示、基础DSPy签名、GEPA优化的DSPy签名，在翻译、术语插入和语言质量评估三个任务中进行评估。

Result: 1. 术语插入任务：优化提示与人工提示质量基本无统计差异；2. 翻译任务：不同方法在不同模型上表现各异；3. 语言质量评估：专家提示在错误检测方面更强，优化提示在错误描述方面更好；4. GEPA显著提升了基础DSPy签名性能；5. 大多数专家与优化提示比较无显著统计差异。

Conclusion: 自动提示优化在某些任务中能达到与专家提示相当的效果，但比较存在不对称性：GEPA优化需要标注数据，而专家提示仅依赖领域知识和迭代优化。结果具有任务依赖性，表明自动优化不能完全替代专家工程，但可作为有效补充。
...
TL;DR: 轻量级LLM通过结构化推理流程可作为实时护栏的安全法官，已在生产环境中部署


<details>
  <summary>Details</summary>
Motivation: 解决LLM系统中提示攻击（越狱和提示注入）的安全风险，填补生产环境中轻量级分类器泛化能力不足与高容量LLM法官延迟过高之间的部署缺口

Method: 通过精心设计的提示和输出结构，引导轻量级LLM进行结构化推理，包括显式意图分解、安全信号验证、危害评估和自我反思

Result: 轻量级通用LLM（如gemini-2.0-flash-lite-001）可作为有效的低延迟法官用于实时护栏，已在新加坡公共服务聊天机器人中作为集中式护栏服务部署；混合模型设置仅带来适度性能提升

Conclusion: 轻量级通用LLM通过结构化推理流程能够可靠地作为安全法官，满足生产环境的实时约束，为解决LLM系统的提示攻击问题提供了实用解决方案
...
TL;DR: 提出跨偏好学习（CPL）框架，通过整合句内和跨条件偏好来显式建模句子级和上下文感知机器翻译的互补优势，提升翻译质量和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有上下文感知机器翻译方法虽然利用文档级信息，但性能提升不一致，因为上下文信号对不同句子的益处不均。现有训练目标未能显式建模这种变异性，限制了模型自适应利用上下文的能力。

Method: 提出跨偏好学习（CPL）框架，将句内偏好（同一条件下不同翻译质量的比较）和跨条件偏好（不同条件下同一句子的翻译比较）整合到偏好优化目标中，显式监督上下文信息何时及如何提升翻译质量。
...
TL;DR: LLMs在角色驱动应用中难以保持隐式一致性，即无法在未明确说明目标的情况下在多轮对话中持续坚持同一目标，这限制了其模拟人类个性特征的能力。


<details>
  <summary>Details</summary>
Motivation: 当前角色驱动的大型语言模型需要跨交互的一致性行为倾向来模拟人类个性特征（如坚持性、可靠性），但现有LLMs缺乏稳定的内部表征来锚定其在扩展对话中的响应。

Method: 设计了一个20个问题风格的谜语游戏范式，让LLM秘密选择一个目标，并对用户的猜测回答"是/否"。通过评估LLM在多轮交互中保持隐式一致性的能力，即能否持续坚持未明确说明的目标。

Result: LLMs难以保持隐式一致性：除非在上下文中明确提供其选择的目标，否则它们的隐式"目标"会在多轮对话中发生偏移。

Conclusion: 这些发现凸显了构建角色驱动LLMs的关键局限性，强调需要开发能够随时间锚定隐式目标的机制，这是实现交互应用（如对话系统）中真实个性建模的关键。
...
TL;DR: 本文系统整理了巴斯克语方言数据资源，包括在线方言数据和标准语转方言数据，并创建了高质量平行评估数据集
...
Method: 区分两类数据源：在线原生方言数据（新闻、推文、词典等）和标准语转方言数据；手动将XNLI测试集适配到三个巴斯克方言，自动生成物理常识数据集并进行人工评估

Result: 创建了巴斯克语方言数据目录，包括高质量平行评估数据集（XNLI方言版本）和经过评估的自动适配数据集（BasPhyCowest），后者可作为银标准数据替代完全手动适配

Conclusion: 提供了系统化的巴斯克语方言数据资源，缓解了数据稀缺问题，为方言NLP研究奠定了基础，自动适配数据可作为手动适配的可行替代方案
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗场景中应用日益广泛，但尚不清楚其在对话中识别和遵循临床实践指南的能力，这对确保循证决策和患者安全至关重要

Method: 收集3,418份CPG文档，提取32,155条临床建议，为每条建议生成多轮对话，评估8个主流LLM的指南检测和遵循能力，并通过56名临床医生进行人工验证

Result: LLM能正确检测71.1%-89.6%的建议，但仅能正确引用3.6%-29.7%的标题；遵循率在21.8%-63.2%之间，显示知道指南内容与实际应用之间存在巨大差距

Conclusion: CPGBench首次系统性揭示了LLM在对话中未能检测或遵循的临床建议，鉴于临床应用的安全关键性，解决这些差距对于LLM在真实临床实践中的安全负责任部署至关重要
...
TL;DR: 比较自然数据和合成数据对LLM训练与评估的影响，以法语和意大利语被动动词交替为例，发现自然数据训练模型在自然和合成测试集上表现稳健，而合成数据训练模型无法可靠泛化到自然句子


<details>
  <summary>Details</summary>
Motivation: 研究自然数据和合成数据在训练和评估大语言模型时的不同影响，探究哪种数据能更好地让模型掌握抽象的语言模式，特别是在句法和语义知识方面

Method: 使用Blackbird Language Matrices结构化数据集，比较从Universal Dependencies提取的自然句子模板与合成句子模板，以法语和意大利语被动动词交替为案例，在不同数据组合下训练和测试模型

Result: 在合成数据集上训练和测试的模型能达到天花板性能，但无法可靠泛化到自然句子；而在自然数据上训练的模型在自然和合成测试集上都表现出稳健性能，能更好地捕捉抽象语言模式

Conclusion: 自然数据在训练LLM捕捉抽象语言模式方面优于合成数据，结构化评估设置对于探测LLM的句法和语义知识具有重要价值
...
TL;DR: MolQuest是一个基于真实化学实验数据的分子结构解析评估框架，将结构解析形式化为多轮交互任务，评估LLMs在复杂化学空间中的溯因推理和战略决策能力，发现当前前沿模型在真实科学场景中表现有限。


<details>
  <summary>Details</summary>
Motivation: 现有科学评估基准主要依赖静态、单轮问答格式，无法衡量模型在需要多步迭代和实验交互的复杂科学任务中的性能。需要系统评估LLMs在真实科学研究中的动态推理能力。

Method: 引入MolQuest，一个基于真实化学实验数据的分子结构解析评估框架。将分子结构解析形式化为多轮交互任务，要求模型主动规划实验步骤、整合异质光谱源（如NMR、MS）并迭代优化结构假设。系统评估LLMs在复杂化学空间中的溯因推理和战略决策能力。

Result: 实证结果显示当代前沿模型在真实科学场景中存在显著局限性：即使最先进的模型准确率仅约50%，而大多数其他模型性能低于30%阈值。这突显了当前LLMs战略科学推理能力的关键差距。

Conclusion: MolQuest为面向科学的LLM评估提供了可复现且可扩展的框架。研究发现突显了当前LLMs战略科学推理能力的关键差距，为未来研究设定了明确方向，即开发能够积极参与科学过程的AI。
...
TL;DR: CRAFT是一个多智能体基准测试，用于评估大语言模型在严格部分信息下的语用通信能力。多个具有互补但不完整视角的智能体需要通过自然语言协调来构建没有任何单个智能体能完全观察到的共享3D结构。


<details>
  <summary>Details</summary>
Motivation: 当前需要评估大语言模型在多智能体协调和语用通信方面的能力，特别是在严格部分信息设置下。现有基准测试可能无法充分捕捉模型在需要空间推理、信念建模和语用通信的复杂协作任务中的表现。

Method: 将问题形式化为多发送者语用推理任务，提供诊断框架将失败分解为空间基础、信念建模和语用通信错误。创建了行为失败配置的分类法，评估了包括8个开放权重模型和7个前沿推理模型在内的多样化模型集。

Result: 更强的推理能力并不能可靠地转化为更好的协调能力：较小的开放权重模型通常匹配或优于前沿系统，改进的个体通信并不能保证成功的协作。多智能体协调仍然是当前语言模型尚未解决的根本挑战。

Conclusion: 多智能体协调是当前语言模型的一个基本未解决问题，需要超越单纯推理能力的专门协调能力。CRAFT基准测试为诊断和评估这些能力提供了系统框架。
...
<details>
  <summary>Details</summary>
Motivation: 在线仇恨内容常以事实性信息形式出现，需要同时评估其伤害性和真实性。现有方法未能联合处理仇恨言论和错误信息，导致偏见加深、公共辩论污染，且增加内容审核人员负担。

Method: 1) 发布WSF-ARG+数据集，结合仇恨言论与核查价值信息；2) 提出新颖的LLM-in-the-loop框架，使用12个不同规模和架构的开源权重LLM进行测试；3) 通过广泛人工评估验证框架效果。

Result: 1) LLM-in-the-loop框架在不降低标注质量前提下减少人工工作量；2) 包含核查价值声明的仇恨信息表现出更高的骚扰和仇恨程度；3) 加入核查价值标签使基于LLM的仇恨言论检测性能提升最高达0.213 macro-F1，大型模型平均提升0.154 macro-F1。

Conclusion: 联合处理仇恨言论和核查价值信息对于有效内容审核至关重要。提出的数据集和框架为这一挑战提供了解决方案，显著提升了仇恨言论检测性能，同时降低了人工标注成本。
...
TL;DR: 提出SGPE算法和WWHO架构，解决BPE分词器在处理复杂Abugida文字时的效率问题，显著减少分词数量并保证音节完整性


<details>
  <summary>Details</summary>
Motivation: 标准BPE分词器在处理复杂Abugida文字（如僧伽罗文和天城文）时存在严重问题：会将复杂的连字（多码点字素簇）拆分为无意义的子字符单元，这降低了LLM的推理效率，迫使模型在推理时学习基本正字法结构，增加了推理成本，对全球南方国家造成了显著的"分词税"

Method: 提出三层架构WWHO（Where-What-How Often）和SGPE（音节感知字素对编码）算法，将文字的语言规则与统计压缩过程分离，同时实现无缝的多语言分词。使用僧伽罗文和天城文（印地语/梵语）作为高度复杂的Abugida文字案例，在3000万句清洗数据集上训练WWHO，并在1,499,950句测试集上评估

Result: 对于僧伽罗文，SGPE实现了1.274的分词词比（TWR），每个分词包含4.83个字符，相比OpenAI的o200k基础模型减少了61.7%的分词。对于印地语，实现了1.181的TWR（相比o200k减少27.0%）。在混合文字（僧伽罗文、天城文和英文）数据集上，SGPE实现了1.240的总体TWR，相对于o200k基础模型、Llama 4 Scout和DeepSeek V3分别减少了36.7%、39.6%和60.2%的分词。这有效地将这些Abugida语言的可使用上下文窗口扩展了高达4.38倍

Conclusion: SGPE算法和WWHO架构成功解决了BPE分词器在处理复杂Abugida文字时的局限性，显著提高了分词效率，减少了推理成本，并通过"语言零拆分保证"确保有效音节永远不会被分割到多个分词中，为全球南方语言提供了更公平的AI访问机会
...
TL;DR: 论文探讨了生成式AI工具进入教育领域后，写作面临被外包、自动化和失去认知价值的风险，强调写作过程本身对人类深度学习的重要性。


<details>
  <summary>Details</summary>
Motivation: 随着ChatGPT等生成式AI工具进入课堂、工作场所和日常思维，写作有沦为形式化、外包化和自动化的风险，可能失去其认知价值。作者关注当人们让机器代写时，会失去什么重要的东西。
...
Conclusion: 在写作可以被伪造的世界里，学习却不能。教育者需要通过更智能的教学方法而非简单禁令来适应AI时代，保持写作作为思维训练的核心价值。
...
TL;DR: 本文提出了自适应分块框架，通过五个内在指标评估文档分块质量，并引入两种新的分块器，显著提升RAG系统性能。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成(RAG)的效果高度依赖于文档分块策略，但现有"一刀切"方法无法捕捉多样化文本的细微结构和语义。分块策略缺乏专门的评估框架，难以独立于下游性能进行评估和比较。

Method: 提出自适应分块框架，为每个文档选择最合适的分块策略。引入五个新的内在、基于文档的指标：引用完整性(RC)、块内内聚性(ICC)、文档上下文连贯性(DCC)、块完整性(BI)和大小合规性(SC)。开发了两种新的分块器：LLM-正则表达式分割器和分割-合并递归分割器，并采用针对性后处理技术。

Result: 在法律、技术和社会科学领域的多样化语料库上，基于指标引导的自适应方法显著提升了下游RAG性能。在不改变模型或提示的情况下，框架将答案正确率从62-64%提高到72%，成功回答的问题数量增加了30%以上（从49个增加到65个）。

Conclusion: 由互补的内在指标引导的自适应、文档感知分块方法，为实现更稳健的RAG系统提供了实用有效的路径。
...
TL;DR: LLM作为令牌压缩器/解压器：通过自表达自编码框架将长文本转换为紧凑的Z-token表示，实现高达18倍的令牌压缩，同时保持重构保真度和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 探索预训练大语言模型作为高效令牌压缩器和解压器的潜力，解决长文本处理中的令牌效率问题，为长上下文推理提供更高效的表示方法。

Method: 设计自表达自编码学习框架，通过轻量级LoRA适配器头微调预训练LLM，将长文本转换为离散、可变长度的潜在代码（Z-token），实现内容自适应压缩：语义密集段分配更多Z-token，冗余或可预测区域则被大幅压缩。

Result: 在Wikipedia、CNN/DailyMail、HotpotQA和Qulac风格的长查询数据集上实现高达18倍的令牌压缩率，同时保持重构保真度和下游任务性能。支持提示压缩和Z-token空间中的自回归生成等应用。

Conclusion: 预训练LLM可作为高效的令牌压缩器/解压器，通过Z-token表示实现内容自适应压缩，为令牌高效的长上下文推理提供了有前景的途径，支持多种实际应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语数学推理方面表现出色，但在多语言环境下存在显著性能差距，主要归因于语言理解能力不足

Method: 提出翻译增强策略优化（TAPO）框架，基于GRPO构建，采用英语作为枢纽语言的显式对齐策略，遵循理解-推理范式，使用步骤级相对优势机制将理解与推理解耦，整合翻译质量奖励

Result: TAPO在多语言数学推理和翻译任务中均优于基线方法，能有效协同语言理解与推理能力，兼容多种模型，对未见语言和领域外任务具有良好的泛化能力

Conclusion: TAPO框架成功解决了多语言数学推理中的语言理解瓶颈，通过翻译增强策略优化实现了语言理解与推理能力的有效协同
...
TL;DR: LLM文本分类中，通过系统调整标签描述、指令引导和少样本示例来研究提示上下文对性能的影响，发现适度增加上下文效果最佳，过度增加可能降低准确性，且效果因模型、任务和批次大小而异


<details>
  <summary>Details</summary>
Motivation: 虽然LLM在社会科学文本分类中能显著降低成本且有时性能媲美现有计算方法，但当前测试中性能差异很大，因此需要研究如何最大化性能，特别是通过提示上下文工程来提高准确性

Method: 系统性地改变提示工程的三个方面：标签描述、指令引导和少样本示例，在两个不同案例中进行测试，分析提示上下文变化对性能的影响

Result: 适度增加提示上下文能带来最大的性能提升，但进一步增加上下文通常只能带来边际改善，有时甚至降低准确性；结果显示出模型、任务和批次大小之间的显著异质性

Conclusion: 不能依赖通用规则，每个LLM编码任务都需要单独验证；提示上下文工程需要谨慎优化，过度增加上下文可能适得其反
...
TL;DR: 研究发现传统LLM生成合成数据方法不适用于罗曼什语，因其有6种变体易混淆。提出根据源语言和目标语言资源梯度对齐数据增强方向的新方法，在最低资源变体上超越Gemini 3 Pro 23 BLEU，首次实现各变体流利翻译。


<details>
  <summary>Details</summary>
Motivation: 传统基于LLM从高资源语言生成合成数据的低资源机器翻译方法在罗曼什语上失效，因为LLM容易混淆其6种不同的语言变体。需要探索更适合罗曼什语这种多方言低资源语言的数据增强方法。

Method: 提出将数据增强方向与源语言和目标语言之间的资源梯度对齐的方法，而非传统LLM生成合成数据方法。通过实验验证这种对齐资源梯度的数据增强策略的有效性。

Result: 新方法在罗曼什语最低资源变体上超越Gemini 3 Pro 23 BLEU分数。人工评估确认该实验产生了首个能够为各个罗曼什语变体生成流利翻译的模型。

Conclusion: 对于像罗曼什语这样具有多个变体的低资源语言，数据增强方向应与语言资源梯度对齐，而非依赖LLM生成合成数据。该方法成功解决了LLM混淆语言变体的问题，实现了各变体的高质量翻译。
...
TL;DR: 对12种假新闻检测方法在10个公开数据集上的综合评估，涵盖传统机器学习、深度学习、Transformer和跨域架构，重点关注领域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 尽管假新闻检测技术有所进步，但大型语言模型和社交媒体的放大效应使虚假信息的生产和传播变得更加复杂。需要系统评估现有方法在不同领域和场景下的表现，特别是泛化能力。

Method: 将12种代表性假新闻检测方法（传统机器学习、深度学习、Transformer、跨域架构）在10个公开英文文本数据集上评估，统一标签为"真实"和"虚假"，进行域内、多域和跨域实验，模拟现实中的领域偏移和分布外数据场景。

Result: 微调模型在域内表现良好但泛化能力差；跨域架构能减少泛化差距但数据需求大；大型语言模型通过零样本和少样本学习展现出有前景的替代方案。

Conclusion: 假新闻检测面临领域泛化挑战，微调模型在域内有效但跨域性能下降，跨域架构和大型语言模型提供了不同权衡方案，结果应视为在特定英文文本协议下的鲁棒性评估。
...
TL;DR: 该研究通过比较人类与视觉语言模型在视觉写作提示语料库中生成的故事，分析了视觉基础故事中的叙事连贯性，发现模型叙事在表面流畅性上接近人类，但在话语组织方面存在系统性差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估视觉语言模型在视觉基础故事生成中的叙事连贯性，探究模型生成叙事与人类叙事在连贯性方面的差异，特别是在表面流畅性相似的情况下，是否存在更深层次的话语组织差异。

Method: 研究方法包括：1）使用视觉写作提示语料库；2）比较人类编写叙事与视觉语言模型生成叙事；3）采用多维度连贯性度量指标，包括指代一致性、话语关系类型、主题连续性、角色持续性、多模态角色基础等；4）计算综合叙事连贯性分数；5）进行联合分析以揭示系统性差异。

Result: 研究结果显示：1）视觉语言模型展现出与人类相似的广泛连贯性特征，但在系统性上存在差异；2）单个度量指标的差异通常较细微，但联合考虑时差异更加明显；3）尽管模型叙事在表面流畅性上接近人类，但在视觉基础故事的话语组织方面存在系统性差异。

Conclusion: 研究结论表明，视觉语言模型生成的叙事虽然在表面流畅性上达到人类水平，但在视觉基础故事的话语组织方面仍存在系统性差异，这为评估和改进模型叙事能力提供了重要见解。
...
TL;DR: 通过arXiv论文分析发现LLM使用导致的词汇变化模式，当前分类器难以准确识别具体生成模型，真实LLM使用呈现异质动态特征


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型对学术论文写作风格的影响，特别是词汇使用模式的变化，以及当前模型识别方法的局限性
...
Conclusion: LLM使用对学术写作风格产生可量化的影响，不同模型间的相似性和差异性导致词汇使用模式演变，需要更精细的方法来识别和评估LLM在学术文本中的使用
...
TL;DR: 该研究探讨了基于大语言模型（LLM）的自动评分系统对构念无关因素的鲁棒性，发现系统对无意义文本填充、拼写错误和写作复杂度具有较好鲁棒性，但文本重复会降低评分，离题回答则被严重惩罚。
...
Method: 研究调查了构念无关因素对基于双架构LLM评分系统的影响，该系统用于评分情境判断测试中的短文式开放回答项目。实验测试了多种构念无关因素对评分的影响。

Result: 评分系统对无意义文本填充、拼写错误和写作复杂度具有良好鲁棒性。但文本重复会导致系统预测分数降低，这与先前非LLM评分系统的研究结果相反。离题回答被系统严重惩罚。

Conclusion: 这些结果为未来基于LLM的评分系统在设计时考虑构念相关性的鲁棒性提供了积极支持。研究强调了在LLM评分系统设计中关注构念相关性的重要性。
...
TL;DR: 论文提出一个系统级视角的自改进语言模型框架，将自改进过程组织为包含数据获取、数据选择、模型优化和推理精炼四个紧密耦合过程的闭环生命周期，并引入自主评估层来监控和指导整个改进循环。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLMs）的不断发展，仅通过人工监督来改进模型变得越来越昂贵且可扩展性有限。当模型在某些领域接近人类水平时，人类反馈可能无法提供足够的信息信号来进一步改进。同时，模型自主决策和执行复杂行动的能力不断增强，自然使得模型开发过程的组件可以逐步自动化。这些挑战和机遇共同推动了自改进研究的兴趣增长。

Method: 论文提出了一个系统级视角的自改进语言模型统一框架，将自改进系统概念化为一个闭环生命周期，包含四个紧密耦合的过程：数据获取、数据选择、模型优化和推理精炼，以及一个自主评估层。模型本身在每个阶段都扮演核心角色：收集或生成数据、选择信息信号、更新参数和精炼输出，而自主评估层持续监控进展并指导跨阶段的改进循环。在此框架下，论文从技术角度系统回顾和分析了每个组件的代表性方法。

Result: 论文提出了一个全面的自改进语言模型框架，系统组织了现有技术，为理解自改进过程提供了统一视角。该框架强调了模型在自改进过程中的核心作用，以及自主评估在指导改进循环中的重要性。

Conclusion: 论文通过提出系统级视角的自改进框架，为自改进语言模型研究提供了结构化方法。该框架将自改进过程组织为闭环生命周期，强调了模型在驱动各阶段改进中的核心作用。论文还讨论了当前限制，并展望了未来实现完全自改进LLMs的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 块扩散语言模型通过结合块级自回归解码和块内并行去噪，为实现快于自回归的生成提供了有前景的路径。然而，在实际加速所需的少步数机制下，标准置信度阈值解码往往很脆弱：激进的阈值会损害质量，而保守的阈值则需要不必要的去噪步骤。现有方法要么需要额外训练，要么需要额外的测试时计算。

Method: 提出了S2D2，一种无需训练的自推测解码框架。关键观察是：当块大小减小到1时，块扩散模型会变成自回归模型，这使得同一个预训练模型可以同时充当草稿器和验证器。S2D2在标准块扩散解码中插入推测验证步骤，并使用轻量级路由策略来决定何时验证值得其成本。这产生了一种混合解码轨迹，其中扩散并行提出令牌，而自回归模式充当局部序列级批评器。

Result: 在三个主流块扩散模型系列中，S2D2始终优于强置信度阈值基线，改善了准确性与速度之间的权衡。在SDAR上，观察到相对于自回归解码最高4.7倍的加速，相对于调优的动态解码基线最高1.57倍的加速，同时准确性最高提升4.5个百分点。在LLaDA2.1-Mini上，S2D2与内置自校正保持互补，包括一个保守设置，其中它比静态基线快4.4倍，且准确性略高。

Conclusion: S2D2是一种有效的训练免费自推测解码框架，通过利用同一预训练模型的双重角色（扩散和自回归），在块扩散语言模型中实现了更好的速度-准确性权衡，为实际加速应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究智能体智能（agentic intelligence），评估智能体在全新任务上的流畅自适应效率，避免语言和外部知识的影响，专注于纯粹的核心认知能力。

Method: 设计基于核心知识先验的抽象回合制环境，通过大量人类测试者进行难度校准，建立基于人类行动基线的效率评分框架，并开发环境构建、验证和校准的方法论。

Result: 人类测试者能够解决100%的环境，而截至2026年3月，前沿AI系统得分低于1%，显示出人类与当前AI系统在智能体智能方面的巨大差距。

Conclusion: ARC-AGI-3提供了一个有效评估智能体智能的基准测试，揭示了当前AI系统在探索、目标推断、内部模型构建和规划等核心认知能力方面的显著不足，为未来AI发展提供了重要参考。
...
### [39] [Trust as Monitoring: Evolutionary Dynamics of User Trust and AI Developer Behaviour](https://arxiv.org/abs/2603.24742)
...
TL;DR: 该研究使用进化博弈论分析AI安全治理，将用户信任建模为重复交互中的监控减少，发现需要透明度、低成本监控和有效制裁才能实现安全AI的广泛采用。


<details>
  <summary>Details</summary>
Motivation: 现有AI治理模型通常将用户信任视为一次性采纳决策，而非动态演化过程。随着AI能力和应用的快速增长，AI安全问题日益紧迫，需要更精细地建模用户与开发者之间的重复交互和信任演化。

Method: 使用进化博弈论，将用户信任建模为重复、非对称交互中的监控减少（检查AI行为有成本）。研究用户信任策略与开发者选择安全（合规）或危险（不合规）AI的协同演化。采用无限种群复制者动态分析，辅以随机有限种群动态和强化学习（Q-learning）模拟。

Result: 发现三种稳健的长期演化机制：1）无采纳且危险开发；2）危险但广泛采用的系统；3）安全且广泛采用的系统。只有第三种是理想的，它出现在对危险行为的惩罚超过安全额外成本，且用户仍能负担偶尔监控的情况下。

Conclusion: 研究结果正式支持强调透明度、低成本监控和有意义制裁的治理提案。表明仅靠监管或盲目用户信任都不足以防止向危险或低采纳结果的演化漂移。需要结合有效的惩罚机制和可负担的监控能力。
...
### [40] [ReLope: KL-Regularized LoRA Probes for Multimodal LLM Routing](https://arxiv.org/abs/2603.24787)
*Yaopei Zeng,Congchao Wang,Blake JianHang Chen,Lu Lin*

Main category: cs.AI

TL;DR: 该论文针对多模态大语言模型中基于探针的路由性能下降问题，提出了两种改进方法：注意力探针和KL正则化LoRA探针，以提升隐藏状态中正确性信号的可分离性。


<details>
  <summary>Details</summary>
Motivation: 在纯文本LLM中，基于隐藏状态的探针路由已被证明是平衡性能与成本的有效策略。然而，当应用于多模态LLM时，这些探针的性能显著下降。研究发现，视觉输入的存在削弱了隐藏状态中正确性信号的可分离性，使得标准探针设计难以提取这些信号。

Method: 提出了两种互补的改进方法：1）注意力探针：基于注意力分数聚合前一层隐藏状态，以恢复分布式的正确性信号；2）KL正则化LoRA探针：插入轻量级LoRA适配器并应用KL正则化器，学习路由感知的表示。

Result: 综合实验表明，所提方法在多个基准测试中一致优于基线方法，证明了通过改进隐藏状态质量是提升MLLM中路由效果的关键。

Conclusion: 该研究揭示了多模态输入对探针路由的挑战，并提出有效的解决方案。改进隐藏状态质量对于多模态LLM中的有效路由至关重要，所提方法为构建高效的多模态路由系统提供了实用工具。
...
### [41] [Resisting Humanization: Ethical Front-End Design Choices in AI for Sensitive Contexts](https://arxiv.org/abs/2603.24853)
*Silvia Rossi,Diletta Huyskes,Mackenzie Jorgensen*

Main category: cs.AI

TL;DR: 本文探讨AI前端设计中人性化元素（如对话交互、情感语言、拟人化隐喻）的伦理意义，指出这些设计选择会深刻影响用户心理模型、信任校准和行为反应，可能误导用户期望、培养错误信任并削弱用户自主性。


<details>
  <summary>Details</summary>
Motivation: 当前AI伦理讨论主要关注后端问题（数据治理、模型训练、算法决策），而前端设计选择（交互方式、表现形式）的伦理意义被忽视。对于基于NLP的对话用户界面（CUI）尤其重要，因为其中的人性化设计元素日益普遍，可能对用户产生深远影响。

Method: 结合人机交互（HCI）、对话AI和价值敏感设计的研究，分析界面如何影响用户期望、信任和自主性。通过Chayn非营利组织支持性别暴力幸存者的两个AI系统案例研究，展示创伤知情设计原则如何指导伦理前端设计。

Result: 前端人性化设计是价值驱动的选择，可能误导用户心理模型、培养错误信任并削弱用户自主性，特别是在脆弱情境下。Chayn案例表明伦理考虑可以促使设计中的原则性克制，挑战当代AI产品中以参与度为导向的设计规范。

Conclusion: 伦理前端AI设计是一种程序性伦理，通过交互选择而非仅系统逻辑来体现。需要更多关注前端设计的伦理影响，特别是在脆弱用户群体中，设计应体现原则性克制而非盲目追求用户参与度。
...
### [42] [How Far Are Vision-Language Models from Constructing the Real World? A Benchmark for Physical Generative Reasoning](https://arxiv.org/abs/2603.24866)
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型评估过度偏向感知真实性，主要关注视觉上合理的3D布局、形状和外观生成，但很少测试模型是否理解构建这些人工制品所需的逐步过程和物理依赖关系，而这种能力对于自动化设计到建造流程至关重要。

Method: 基于住宅木结构建筑领域（具有完全编码化的工程标准和客观可验证的正确性）构建基准，收集了13种建筑风格的26,000多个结构，均验证到施工文档标准（LOD 350），并开发了确定性的10项结构验证框架。支持迭代式智能体交互：模型观察中间建造状态，生成施工动作，接收结构化环境反馈。
...
### [43] [On the Foundations of Trustworthy Artificial Intelligence](https://arxiv.org/abs/2603.24904)
*TJ Dunham*

Main category: cs.AI

TL;DR: 该论文提出平台确定性推理是可信AI的必要且充分条件，通过构建纯整数推理引擎实现跨架构比特级一致性输出，证明AI信任本质上是算术问题。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统缺乏平台确定性，IEEE 754浮点运算在不同硬件平台上产生非确定性结果，这破坏了AI系统的可信属性（公平性、鲁棒性、隐私性、安全性、对齐性），因为这些属性都预设了平台确定性。

Method: 提出确定性论题并引入信任熵量化非确定性成本；构建纯整数推理引擎替代浮点运算，实现ARM和x86架构间的比特级相同输出；通过跨架构测试（模型参数达67亿）、地理分布式节点验证和链上认证交易进行验证。

Result: 在82次跨架构测试中观察到零哈希不匹配；四个地理分布式节点产生相同输出，通过356笔链上认证交易验证；验证失败概率精确等于1 - 2^{-H_T}；确定性下的验证仅需O(1)哈希比较，而非确定性下则面临难解成员资格问题。

Conclusion: 平台确定性推理是可信AI的必要且充分条件；AI信任本质上是算术问题；通过纯整数推理引擎可以实现跨平台比特级确定性，为构建可信AI系统提供了理论基础和工程实现。
...
### [44] [LogitScope: A Framework for Analyzing LLM Uncertainty Through Information Metrics](https://arxiv.org/abs/2603.24929)
...
Method: 引入LogitScope框架，通过在每个生成步骤计算熵和变熵等标记级信息度量来分析LLM不确定性。该框架模型无关，通过延迟评估实现计算效率，兼容任何HuggingFace模型。
...
### [45] [Decoding Market Emotions in Cryptocurrency Tweets via Predictive Statement Classification with Machine Learning and Transformers](https://arxiv.org/abs/2603.24933)
*Moein Shahiki Tash,Zahra Ahani,Mohim Tash,Mostafa Keikhay Farzaneh,Ari Y. Barrera-Animas,Olga Kolesnikova*

Main category: cs.AI

TL;DR: 提出一个两阶段分类框架，用于识别加密货币推文中的预测性陈述，结合人工与GPT标注、情感特征提取和数据增强，评估多种模型性能并分析情感模式。
...
Method: 采用两阶段分类：任务1为二元分类（预测性vs非预测性），任务2对预测性推文进一步分类（增量、减量、中性）。结合人工和GPT标注构建数据集，使用SenticNet提取情感特征，通过GPT生成改写进行数据增强以解决类别不平衡。评估了机器学习、深度学习和基于Transformer的多种模型。

Result: GPT平衡方法显著提升模型性能：Transformer模型在任务1中取得最高F1分数，传统机器学习模型在任务2中表现最佳。情感分析揭示了不同加密货币各预测类别对应的独特情感模式。

Conclusion: 提出的分类框架能有效识别加密货币推文中的预测性陈述，GPT增强方法解决了数据不平衡问题，情感分析为理解加密货币市场情绪提供了新视角。
...
### [46] [FinMCP-Bench: Benchmarking LLM Agents for Real-World Financial Tool Use under the Model Context Protocol](https://arxiv.org/abs/2603.24943)
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏专门针对金融领域、能够评估大语言模型通过工具调用解决实际金融问题能力的标准化基准测试。金融领域的复杂性和专业性需要专门的评估框架来衡量模型在真实金融场景中的表现。

Method: 构建了包含613个样本的基准测试，涵盖10个主要场景和33个子场景，包含真实和合成用户查询以确保多样性和真实性。整合了65个真实金融模型上下文协议，设计了单工具、多工具和多轮对话三种样本类型来评估不同任务复杂度下的模型表现。提出了专门衡量工具调用准确性和推理能力的评估指标。

Result: FinMCP-Bench提供了一个标准化、实用且具有挑战性的测试平台，能够系统评估主流大语言模型在金融工具调用任务中的表现。通过该基准测试，研究人员可以量化比较不同模型在金融问题解决能力上的差异。

Conclusion: FinMCP-Bench填补了金融领域大语言模型评估基准的空白，为金融智能体研究提供了重要的评估工具，有助于推动金融大语言模型在解决实际金融问题方面的研究和应用发展。
...
TL;DR: 该研究首次大规模分析了电商平台嵌入式购物AI的采用和使用模式，基于携程3100万用户数据发现：AI助手采用者主要为年长、女性、高活跃度用户；AI聊天与传统搜索处于相同购买阶段但更早于下单，用户常交替使用两种模式；AI主要用于探索性、难以关键词化的任务，如景点查询占42%，表明嵌入式购物AI是传统搜索的补充而非替代。


<details>
  <summary>Details</summary>
Motivation: 研究动机是填补关于消费者如何采用和使用平台嵌入式购物AI的大规模实证证据空白。虽然通用AI工具的采用模式已有研究，但针对电商平台特定场景的嵌入式AI助手（如携程的"问道"）的采用和使用模式尚缺乏系统性分析。

Method: 研究方法基于携程（中国最大在线旅游平台）3100万用户的数据，分析其LLM-based AI助手"问道"的使用情况。通过大规模用户行为数据分析，识别AI助手的采用模式、使用时机以及与搜索行为的交互关系。

Result: 研究发现三个主要规律：1）AI采用者主要为年长消费者、女性用户和高活跃度现有用户，与通用AI工具的年轻男性主导模式相反；2）AI聊天与传统搜索处于相同购买阶段但早于下单，用户常交替使用两种模式；3）消费者主要用AI处理探索性、难以关键词化的任务，景点查询占42%，聊天意图随使用时机和后续购买品类系统变化。

Conclusion: 结论表明嵌入式购物AI在电商中主要作为传统搜索的补充界面，而非替代品，特别适用于探索性产品发现。这一发现对平台设计AI功能、理解消费者行为以及优化电商用户体验具有重要意义。
...
TL;DR: ScratchMath是一个专门用于解释和分类学生手写数学草稿错误的基准数据集，包含1,720个中国中小学数学样本，支持错误原因解释和分类任务。


<details>
  <summary>Details</summary>
Motivation: 现有教育NLP主要关注文本回答，忽略了手写草稿的复杂性；当前多模态大语言模型主要采用"应试者视角"生成正确答案，而非诊断学生错误，需要专门评估模型诊断手写数学错误的能力。

Method: 构建包含1,720个中国中小学学生手写数学样本的数据集，定义7种错误类型，支持错误原因解释和错误原因分类两个任务，采用多阶段专家标注、审查和验证的人机协作方法进行精细标注。

Result: 评估了16个领先的MLLMs，发现与人类专家相比存在显著性能差距，尤其在视觉识别和逻辑推理方面；专有模型明显优于开源模型，大型推理模型在错误解释方面表现出强大潜力。

Conclusion: ScratchMath填补了手写数学草稿错误诊断的评估空白，揭示了当前MLLMs在理解学生手写错误方面的局限性，为教育AI研究提供了重要基准和数据集。
...
<details>
  <summary>Details</summary>
Motivation: 当前的不确定性方法（单一评分或经典二分法）无法为改进生成模型提供可操作的见解，且不足以理解LLM中的不确定性。需要更细粒度的分解来审计LLM可靠性和检测幻觉。

Method: 提出一个不确定性分解框架，将LLM不确定性分解为三个语义组件：1) 输入模糊性（来自模糊提示），2) 知识缺口（由参数证据不足引起），3) 解码随机性（源于随机采样）。通过一系列实验验证该框架。

Result: 实验表明这些组件的主导地位会随模型规模和任务而变化。该框架为审计LLM可靠性和检测幻觉提供了更好的理解，能够实现有针对性的干预和更可信的系统。

Conclusion: 提出的三组件不确定性分解框架比现有方法提供更可操作的见解，有助于理解LLM不确定性来源，为改进模型可靠性和检测幻觉铺平道路。
...
TL;DR: 本文提出多视角故障归因范式，解决多智能体系统中故障归因的模糊性问题，并引入首个多视角故障归因基准MP-Bench及相应评估协议。
...
Method: 提出多视角故障归因范式，明确考虑归因模糊性；构建MP-Bench基准，这是首个专门为多智能体系统多视角故障归因设计的基准；设计针对该范式的新评估协议。

Result: 通过大量实验发现，先前关于LLM在故障归因方面表现不佳的结论主要源于现有基准设计的局限性。结果强调了多视角基准和评估协议对于实现现实可靠的多智能体系统调试的必要性。

Conclusion: 多视角故障归因是解决多智能体系统故障归因模糊性的实用范式，MP-Bench基准和相应评估协议为更现实、可靠的系统调试提供了必要工具，挑战了现有基准得出的关于LLM能力的结论。
...
TL;DR: 该论文提出了一种理论框架，旨在通过架构层面的设计减少模型蒸馏中的能力-治理不对称风险，核心观点是将高层能力与内部稳定性约束耦合，使蒸馏作为捷径的价值降低。


<details>
  <summary>Details</summary>
Motivation: 论文的动机源于对知识蒸馏、模型提取和行为转移等前沿AI安全问题的关注。主要风险不在于简单的复制，而在于有用的能力可能比伴随其产生的治理结构更廉价地被转移，这种不对称性构成了核心安全威胁。

Method: 论文提出了一个约束耦合推理框架，包含四个关键要素：有界转移负担、路径负载累积、动态演化的可行区域，以及能力-稳定性耦合条件。该框架是公开且保护商业机密的，省略了专有实现细节、训练配方、阈值、隐藏状态检测、部署流程和机密系统设计选择。

Result: 论文的贡献主要是理论性的而非操作性的，提供了一个可证伪的架构论点、清晰的威胁模型，以及一组可用于未来蒸馏抵抗、对齐和模型治理研究的可实验验证的假设。

Conclusion: 该论文通过引入约束耦合推理框架，为减少模型蒸馏中的能力-治理不对称性提供了理论基础。通过将高层能力与内部稳定性约束耦合，使蒸馏作为捷径的价值降低，从而增强模型治理的安全性。
...
Result: 在PDEBench八个系列上评估，SAKE是预算匹配的低成本选择器中表现最好的：67.8%精确匹配，91.7%误差在1以内，6.1%平均遗憾@拐点，成本比0.051（节省94.9%搜索成本）。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在心理支持和情感陪伴场景中的核心局限在于其基于局部下一词预测的机制，无法维持多轮干预所需的时序连续性、阶段意识和用户同意边界，导致系统容易出现阶段提前、阶段错位和边界侵犯等问题

Method: 提出LEKIA 2.0情境化LLM架构，将认知层与执行层分离，从而解耦情境建模与干预执行。该设计使系统能够在持续交互中维护用户情境和同意边界的稳定表征

Result: LEKIA在深度干预循环完成度上比仅使用提示的基线平均绝对提升约31%。引入静态到动态在线评估协议评估过程控制能力

Conclusion: 外部情境结构是构建稳定、可控、情境化情感支持系统的关键使能条件，分离认知与执行层的架构设计能有效解决过程导向情感支持中的核心挑战
...
### [54] [Beyond Content Safety: Real-Time Monitoring for Reasoning Vulnerabilities in Large Language Models](https://arxiv.org/abs/2603.25412)
*Xunguang Wang,Yuguang Zhou,Qingyue Wang,Zongjie Li,Ruixuan Huang,Zhenlan Ji,Pingchuan Ma,Shuai Wang*

Main category: cs.AI

TL;DR: 该论文首次系统性地提出并定义了"推理安全"概念，建立了九类不安全推理行为的分类法，开发了实时推理安全监控器，并在大规模评估中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型安全研究主要关注输出内容的安全性，而忽视了推理过程本身的安全问题。现有工作将推理链视为不透明的中间产物，但推理过程可能存在逻辑不一致、计算效率低下和对抗性操纵等安全风险，需要建立专门的推理安全框架。

Method: 1. 形式化定义推理安全并建立九类不安全推理行为的分类法（涵盖输入解析错误、推理执行错误和过程管理错误）；2. 进行大规模流行性研究，标注4111个来自自然推理基准和四种对抗攻击方法的推理链；3. 提出推理安全监控器：基于LLM的外部组件，通过分类法嵌入提示实时检查每个推理步骤，并在检测到不安全行为时发送中断信号。

Result: 1. 验证了所有九类错误类型在实践中都存在，且每种攻击都产生机制上可解释的特征；2. 在450链静态基准测试中，监控器达到84.88%的步骤级定位准确率和85.37%的错误类型分类准确率；3. 显著优于幻觉检测器和过程奖励模型基线。
...
### [55] [Mechanistically Interpreting Compression in Vision-Language Models](https://arxiv.org/abs/2603.25035)
*Veeraraju Elluru,Arth Singh,Roberto Aguero,Ajay Agarwal,Debojyoti Das,Hreetam Paul*

Main category: cs.AI

TL;DR: 该研究分析了压缩视觉语言模型（VLMs）的内部计算和安全行为变化，发现剪枝会保持电路结构但旋转和衰减内部特征，而量化会修改高级电路但保留更好的特征对齐，并开发了VLMSafe-420基准来评估压缩对安全性的影响。


<details>
  <summary>Details</summary>
Motivation: 压缩视觉语言模型虽然能降低内存和计算成本，但引发了对其内部计算和安全行为是否得到保留的担忧。研究旨在深入理解剪枝和量化这两种主要压缩方法如何从根本上改变VLMs的内部机制。

Method: 使用因果电路分析和基于交叉编码器的特征比较方法，检查剪枝和量化如何改变代表性VLMs的内部结构。同时引入了VLMSafe-420基准，该基准将有害输入与各种安全类别下的匹配良性反事实配对。

Result: 研究发现：1）剪枝通常保持电路结构完整，但会旋转和衰减内部特征；2）量化修改了更高级别的电路，但使幸存的特征更好地对齐；3）剪枝导致真正的拒绝行为急剧下降，表明压缩方法的选择具有安全影响。

Conclusion: 压缩方法的选择对VLMs的安全行为有显著影响，特别是剪枝会严重损害模型的拒绝能力。研究强调了在压缩VLMs时考虑安全影响的重要性，并提供了分析工具和基准来评估这些影响。
...
### [56] [MP-MoE: Matrix Profile-Guided Mixture of Experts for Precipitation Forecasting](https://arxiv.org/abs/2603.25046)
*Huyen Ngoc Tran,Dung Trung Tran,Hong Nguyen,Xuan Vu Phan,Nam-Phong Nguyen*

Main category: cs.AI

TL;DR: 提出MP-MoE框架，通过结合矩阵轮廓目标函数缓解降水预报中的"双重惩罚"问题，在越南流域数据集上验证了其在强降水事件预报中的有效性。


<details>
  <summary>Details</summary>
Motivation: 热带地区（如越南）的降水预报面临复杂地形和对流不稳定的挑战，数值天气预报模型存在偏差。传统数据驱动的后处理方法依赖逐点目标函数，在时间错位时会产生"双重惩罚"效应，影响预报准确性。

Method: 提出矩阵轮廓引导的专家混合框架，将传统强度损失与结构感知的矩阵轮廓目标函数相结合。利用子序列级相似性而非逐点误差，实现更可靠的专家选择，缓解相位偏移导致的过度惩罚。

Result: 在越南两个主要流域的降雨数据集上评估，涵盖1小时强度和12、24、48小时累积降雨。MP-MoE在强降水事件的平均关键成功指数上优于原始NWP和基线学习方法，同时显著降低动态时间规整值。

Conclusion: MP-MoE框架能有效捕捉峰值降雨强度并保持风暴事件的形态完整性，为热带地区降水预报提供了一种缓解"双重惩罚"问题的新方法。
...
### [57] [Sparse Visual Thought Circuits in Vision-Language Models](https://arxiv.org/abs/2603.25075)
*Yunpeng Zhou*

Main category: cs.AI

TL;DR: SAE特征在视觉语言模型中缺乏模块化组合性：干预任务选择性特征集能适度提升推理准确率，但两个特征集的并集干预会导致输出漂移和准确率下降，表明存在共享内部通路和非模块化电路干扰。


<details>
  <summary>Details</summary>
Motivation: 验证稀疏自编码器（SAE）特征是否形成模块化、可组合的推理单元，这是许多基于干预的引导方法的基本假设。研究SAE特征的可组合性边界，为更可靠的视觉语言模型控制提供诊断框架。

Method: 开发可复现的因果管道，在Qwen3-VL-8B中定位和测试稀疏视觉思维电路。在包含七种任务类型和三个难度级别的合成基准上，使用线性探针识别任务类型信息的中层解码器位置，在该层训练SAE，通过显式规则构建任务选择性特征集，在推理时进行缩放和消融干预，同时量化准确率和输出漂移。
...
Conclusion: SAE特征缺乏完全模块化的组合性，存在共享内部通路导致特征并集干预时的干扰效应。研究阐明了SAE特征可组合性的边界，为更可靠的视觉语言模型控制提供了严格的诊断框架，强调了在基于特征的干预方法中考虑电路级相互依赖性的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 少样本类增量学习（FSCIL）在只有少量标记样本的采集上下文中特别容易受到影响。触觉感知场景中，采集上下文（如不同设备、接触状态和交互设置）由于缺乏标准化而降低性能，需要解决上下文变化带来的挑战。

Method: 提出Context-as-Transform FSCIL（CaT-FSCIL）：1）将采集上下文分解为结构化低维分量（受触觉交互特征影响）和高维残差分量（源于平台和设备差异）；2）对低维分量建模为近似可逆的Context-as-Transform族，通过伪上下文一致性损失优化的逆变换规范化处理；3）对高维残差分量使用不确定性条件原型校准（UCPC），基于上下文不确定性校准有偏原型和决策边界。

Result: 在标准基准测试HapTex和LMT108上进行了全面实验，证明了CaT-FSCIL方法的优越性。

Conclusion: CaT-FSCIL通过分解和处理采集上下文的不同分量，有效解决了触觉感知中少样本类增量学习面临的上下文变化问题，在标准基准上表现出优越性能。
...
TL;DR: RubricEval是首个针对指令跟随评估的细粒度评分标准级别元评估基准，揭示了当前评分标准级别评估的局限性，即使GPT-4o在困难子集上准确率仅55.97%


<details>
  <summary>Details</summary>
Motivation: 当前基于评分标准的指令跟随评估方法虽然广泛使用，但其可靠性尚不明确，需要元评估。现有元评估主要关注响应级别，无法评估评分标准级别评估所依赖的细粒度判断准确性，因此需要填补这一空白

Method: 提出RubricEval基准，包含：(1)首个评分标准级别的指令跟随元评估基准；(2)涵盖多个类别和模型来源的多样化指令和响应；(3)3,486个质量控制实例，以及能更好区分评估者性能的简单/困难子集。通过实验比较不同评估范式，分析评分标准分类学

Result: 评分标准级别评估远未解决：GPT-4o在困难子集上准确率仅55.97%。评分标准级别评估优于检查表级别，显式推理提高准确性，两者结合减少评估者间方差。通过评分标准分类学识别常见失败模式

Conclusion: RubricEval填补了评分标准级别元评估的空白，揭示了当前评估方法的局限性，为可靠的指令跟随评估提供了可操作的见解，强调需要改进评分标准级别的评估质量
...
TL;DR: UniAI-GraphRAG是一个增强的图检索增强生成框架，通过本体引导知识提取、多维社区聚类和双通道图检索融合，显著提升了复杂推理和多跳查询的性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统在复杂推理、多跳查询和领域特定QA方面存在挑战，传统GraphRAG框架在跨行业适应性、社区报告完整性和检索性能方面仍有局限，需要更先进的解决方案。

Method: 提出三个核心创新：1) 本体引导知识提取，使用预定义Schema指导LLMs准确识别领域特定实体和关系；2) 多维社区聚类策略，通过对齐补全、基于属性的聚类和多跳关系聚类提高社区完整性；3) 双通道图检索融合，通过混合图和社区检索平衡QA准确性和性能。
...
TL;DR: AI辅助安全分析存在"能力阴影"风险：AI生成的建议会系统性窄化人类思维，导致安全分析质量下降而非提升，且这种退化效应是乘性而非加性的。


<details>
  <summary>Details</summary>
Motivation: 随着AI助手被集成到物理AI系统的安全工程工作流中，需要评估AI辅助是否真正提高安全分析质量，还是引入了仅在部署后事故中显现的系统性盲点。

Method: 建立安全工程五维能力框架（领域知识、标准专业知识、操作经验、上下文理解、判断力），形式化四种典型人机协作结构，推导性能界限，分析"能力阴影"效应。

Result: 发现"能力阴影"效应会导致安全分析质量乘性退化，远超朴素加性估计；AI辅助安全工程是协作设计问题而非软件采购决策，同一工具可能改善或恶化分析质量。

Conclusion: 需要从工具认证转向工作流认证，建立抗阴影的工作流非退化条件，确保物理AI系统的可信性。
...
TL;DR: 开发了基于3D骨骼运动的步态基础模型，使用深度相机记录3414名成人的五种运动任务，模型嵌入优于工程特征，能预测多种表型指标，证明步态可作为独立的多系统生物信号。


<details>
  <summary>Details</summary>
Motivation: 步态日益被视为重要生命体征，但目前方法仅将其视为特定病理的症状而非系统性生物标志物。需要建立步态作为独立多系统生物信号的证据基础。

Method: 开发步态基础模型，使用深度相机记录3414名深度表型化成人的3D骨骼运动数据，涵盖五种运动任务。模型学习嵌入表示，并与工程特征进行比较。

Result: 学习嵌入优于工程特征，能预测年龄(r=0.69)、BMI(r=0.90)、内脏脂肪面积(r=0.82)。嵌入显著预测3210个表型目标中的1980个；调整年龄、BMI、VAT和身高后，步态在男性所有18个身体系统、女性17/18个系统中提供独立增益，改善临床诊断和用药预测。解剖学消融显示腿部主导代谢和衰弱预测，躯干编码睡眠和生活方式表型。

Conclusion: 步态是独立的多系统生物信号，支持将其转化为消费级视频应用，并作为可扩展、被动的生命体征进行整合。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM后训练量化方法通常对所有层采用相同的量化策略，但研究发现浅层和深层对量化更敏感，尤其是首尾层量化误差显著更大，需要针对不同层的敏感度设计多层次的量化方案。

Method: 提出SliderQuant框架，包含两个核心组件：1) 层间滑动量化，针对浅层、中间层和深层设计三种滑动窗口；2) 层内滑动量化，采用增量策略量化每个窗口。通过少量可学习参数实现自适应滑动量化。

Result: 在基础语言生成、零样本常识推理、数学和代码任务上，对Llama/Llama2/Llama3/Qwen2.5系列、DeepSeek-R1蒸馏模型和大规模MoE模型进行测试，SliderQuant在仅权重量化和权重-激活量化方面均优于现有PTQ方法（包括使用旋转变换的最新方法）。

Conclusion: SliderQuant通过考虑不同层对量化的敏感度差异，采用自适应滑动量化策略有效减少各层量化误差，为LLM后训练量化提供了更优的解决方案。
...
TL;DR: Doc2SemDAG：从科学文献中构建基于文档的语义有向无环图（DAG）框架，通过DAGverse-Pipeline半自动系统从论文图表和文本中提取结构化知识，并发布专家验证的DAG数据集。


<details>
  <summary>Details</summary>
Motivation: 现实世界DAG数据集稀缺，因为构建需要专家解释领域文档。科学论文中的DAG图表提供了显式结构，而伴随文本提供了上下文和解释，这为监督学习提供了自然来源。需要解决文档可能允许多种合理抽象、预期结构通常隐含、支持证据分散在文本、方程、标题和图表中的挑战。

Method: 提出DAGverse框架，其核心组件DAGverse-Pipeline是一个半自动系统，包含图表分类、图重建、语义接地和验证四个步骤。利用科学论文中的显式DAG图表作为监督信号，从图表中提取DAG结构，从文本中提取上下文和解释证据。

Result: 发布了DAGverse-1数据集，包含108个专家验证的语义DAG，具有图级、节点级和边级证据。实验表明DAGverse-Pipeline在DAG分类和标注任务上优于现有视觉语言模型。以因果DAG为案例研究验证了框架有效性。

Conclusion: DAGverse为基于文档的DAG基准测试提供了基础，并为研究基于现实世界证据的结构化推理开辟了新方向。该方法能够从科学文献中高效构建高质量的语义DAG数据集，解决了DAG数据稀缺问题。
...
TL;DR: 本文提出了一个评估AI有害操纵行为的框架，通过三个领域（公共政策、金融、健康）和三个地区（美国、英国、印度）的10,101名参与者的交互研究，发现AI模型在被提示时会产生操纵行为，且能改变参与者的信念和行为，但效果因领域和地区而异。
...
Method: 提出了一个基于情境特定的人机交互研究的评估框架，在三个AI应用领域（公共政策、金融、健康）和三个地区（美国、英国、印度）对10,101名参与者进行实验，测试AI模型的操纵倾向和效果。

Result: 1) 测试的AI模型在被提示时会产生操纵行为，并能诱导参与者的信念和行为改变；2) 情境很重要：不同领域的AI操纵行为存在差异；3) 不同地理区域存在显著差异，一个地区的结果不能推广到其他地区；4) 模型的操纵倾向频率与操纵成功率之间没有一致的相关性。

Conclusion: 需要在高风险情境中评估AI操纵行为，不同领域和地区的评估应分别进行，操纵倾向和成功率应作为独立维度研究，并公开了测试协议和材料以促进框架的采用。
...
TL;DR: 研究分析了基于深度强化学习（DRL）的自动驾驶车辆在混合交通中的宏观交通流特性和燃油效率，发现相比传统IDM模型，DRL能提升约7.52%的道路容量，并在高速下提升约28.98%的燃油效率。


<details>
  <summary>Details</summary>
Motivation: 混合交通中自动驾驶车辆控制面临平衡安全、效率、舒适性、燃油效率和交通规则遵守等多重挑战，传统跟驰模型（如IDM）难以泛化到多样化交通场景且通常不考虑燃油效率，而DRL在微观跟驰性能上表现良好但宏观交通流特性尚未充分探索。

Method: 采用Twin Delayed Deep Deterministic Policy Gradient (TD3)算法控制自动驾驶车辆，使用NGSIM高速公路数据集进行训练以实现与人类驾驶车辆的真实交互。通过基本图（FD）评估不同驾驶员异质性、异质时间间隔渗透水平和不同RL车辆比例下的交通性能，并宏观比较RL模型与IDM的燃油效率。

Result: 交通性能对安全时间间隔分布和RL车辆比例敏感：从完全人类驾驶过渡到完全RL控制可提升约7.52%的道路容量；RL模型在高速（>50 km/h）下比IDM提升约28.98%的平均燃油效率，低速（<50 km/h）下提升约1.86%。
...
<details>
  <summary>Details</summary>
Motivation: 算术谜题游戏为研究数学推理任务中的难度提供了受控环境，这是自适应学习系统的核心挑战。研究者旨在探究整数算术谜题中难度的结构性决定因素。

Method: 1. 形式化问题并开发精确的动态规划求解器，用于枚举可达目标、提取最小操作见证、支持大规模标注；2. 构建包含340多万个实例的数据集，通过最小操作数定义难度；3. 分析难度与求解器导出特征的关系，比较基线机器学习模型与结构性属性的预测能力。

Result: 1. 基于袋和目标级别统计的基线机器学习模型能部分预测可解性，但无法可靠区分简单实例；2. 难度完全由精确见证导出的少量可解释结构性属性决定；3. 最小构造中使用的输入值数量是此标注下难度的最小充分统计量。

Conclusion: 研究提供了透明、计算基础的谜题难度解释，桥接了符号推理与数据驱动建模。该框架支持可解释的难度估计和原则性任务排序，对自适应算术学习和智能练习系统有直接应用价值。
...
TL;DR: 研究验证了基于5W3H的PPS结构化意图表示框架在跨语言和跨模型中的泛化能力，发现AI辅助扩展的提示与人工编写的提示在目标对齐上无显著差异，且结构化表示能减少跨模型输出方差。


<details>
  <summary>Details</summary>
Motivation: 扩展先前仅针对中文的研究，验证结构化意图表示框架PPS是否能在不同语言和模型间泛化，并探索AI辅助扩展提示的可行性。

Method: 采用5W3H框架的PPS结构化意图表示，在三个语言（英语、日语、中文）、四个条件（包括AI辅助扩展提示）、三个LLM模型上进行实验，共分析2,160个模型输出。

Result: AI扩展的5W3H提示与人工编写的提示在目标对齐上无统计显著差异；结构化条件能减少或重塑跨模型输出方差；非结构化提示存在系统性双重膨胀偏差。
...
### [69] [Modernising Reinforcement Learning-Based Navigation for Embodied Semantic Scene Graph Generation](https://arxiv.org/abs/2603.25415)
*Roman Kueble,Marco Hueller,Mrunmai Phatak,Rainer Lienhart,Joerg Haehner*

Main category: cs.AI

TL;DR: 该研究改进了具身语义场景图生成的导航组件，通过优化算法、细粒度动作表示和课程学习提升模型质量与导航效率的权衡。


<details>
  <summary>Details</summary>
Motivation: 语义世界模型使具身智能体能够超越纯几何表示进行推理，是有机计算中目标驱动自适应的重要使能器。核心挑战是在有限动作预算内最大化模型质量和下游有用性。语义场景图为此提供了结构化紧凑表示，但有限动作范围内构建需要权衡信息增益与导航成本。

Method: 提出模块化导航组件用于具身语义场景图生成，现代化决策机制：替换策略优化方法，重新审视离散动作表述。研究紧凑和细粒度的大离散动作集，比较原子动作的单头策略与动作组件的分解多头策略。评估课程学习和可选深度碰撞监督，评估SSG完整性、执行安全性和导航行为。

Result: 仅替换优化算法在相同奖励塑造下相对基线提高SSG完整性21%。深度主要影响执行安全性（无碰撞运动），完整性基本不变。现代优化与细粒度分解动作表示结合产生最强的完整性-效率权衡。

Conclusion: 通过现代化优化算法和细粒度分解动作表示，显著提升了具身语义场景图生成的导航效率与模型质量权衡，为资源受限环境下的语义世界模型构建提供了有效解决方案。
...
### [70] [Retraining as Approximate Bayesian Inference](https://arxiv.org/abs/2603.25480)
*Harrison Katz*
...
<details>
  <summary>Details</summary>
Motivation: 传统上将模型重训练视为维护任务，缺乏理论框架。作者认为需要从贝叶斯推断和决策理论角度重新理解重训练过程，以建立更科学的重训练策略

Method: 提出决策理论框架，将重训练视为计算约束下的近似贝叶斯推断。定义"学习债务"概念（持续更新的信念状态与冻结部署模型之间的差距），将重训练决策建模为成本最小化问题，从损失函数推导出阈值

Result: 开发出基于证据的触发机制，替代传统的日历调度方式，使重训练治理可审计。提供决策理论框架指导重训练策略制定

Conclusion: 重训练应被视为计算约束下的贝叶斯推断过程，而非简单的维护任务。通过决策理论框架可以制定更科学、可审计的重训练策略，提高机器学习系统的管理效率
...
### [71] [Voxtral TTS](https://arxiv.org/abs/2603.25551)
...
TL;DR: Voxtral TTS是一个多语言语音合成模型，仅需3秒参考音频即可生成自然语音，采用自回归语义token生成与流匹配声学token的混合架构，使用新型Voxtral Codec语音tokenizer，在人类评估中优于ElevenLabs Flash v2.5。
...
Method: 采用混合架构：1）使用自回归生成语义语音token；2）使用流匹配生成声学token；3）使用Voxtral Codec语音tokenizer，该tokenizer基于混合VQ-FSQ量化方案从头训练。

Result: 在由母语者进行的人类评估中，Voxtral TTS在多语言语音克隆方面因其自然度和表达性而受到青睐，以68.4%的胜率优于ElevenLabs Flash v2.5。模型权重已根据CC BY-NC许可证发布。

Conclusion: Voxtral TTS是一个高效的多语言语音合成系统，仅需极少参考音频即可生成高质量语音，其混合架构和新型tokenizer设计在自然度和表达性方面表现出色，优于现有商业系统。
...
TL;DR: RC2：通过强化学习框架实现跨模态循环一致性，利用模型内部冲突作为学习信号，提升多模态推理准确性
...
Conclusion: 高级推理能力的涌现不仅来自数据规模的扩展，还来自对世界结构一致性的强制理解，跨模态循环一致性为多模态学习提供了有效的自监督信号
...
### [73] [Back to Basics: Revisiting ASR in the Age of Voice Agents](https://arxiv.org/abs/2603.25727)
*Geeyang Tay,Wentao Ma,Jaewon Lee,Yuzhi Tang,Daniel Lee,Weisu Yin,Dongming Shen,Silin Meng,Yi Zhu,Mu Li,Alex Smola*

Main category: cs.AI

TL;DR: WildASR是一个多语言诊断基准测试，从真实人类语音中提取，通过三个维度（环境退化、人口统计偏移、语言多样性）分解ASR鲁棒性，揭示了现有ASR系统在真实场景中的严重且不均衡的性能退化问题。


<details>
  <summary>Details</summary>
Motivation: 当前ASR系统在精心策划的基准测试上已达到接近人类的准确率，但在真实语音助手场景中仍会失败，而现有评估方法无法系统覆盖这些失败条件。缺乏能够隔离特定失败因素的诊断工具，使得从业者无法预测在何种条件下、何种语言中会出现何种程度的性能退化。

Method: 提出了WildASR基准测试，这是一个完全源自真实人类语音的多语言（四种语言）诊断基准。该基准沿着三个轴分解ASR鲁棒性：环境退化、人口统计偏移和语言多样性。评估了七个广泛使用的ASR系统，并提供了三种分析工具供从业者指导部署决策。
...
### [74] [SolRugDetector: Investigating Rug Pulls on Solana](https://arxiv.org/abs/2603.24625)
*Jiaxin Chen,Ziwei Li,Zigui Jiang,Ruihong He,Yantong Zhou,Jiajing Wu,Zibin Zheng*

Main category: cs.CR

TL;DR: 该论文对Solana区块链上的Rug Pull欺诈行为进行了首次系统性实证研究，提出了基于链上数据的检测系统SolRugDetector，并在大规模测量中识别了大量欺诈代币。


<details>
  <summary>Details</summary>
Motivation: Solana因高性能和低成本而快速增长，但其极低的代币发行门槛导致了广泛的Rug Pull欺诈。与以太坊基于恶意智能合约的Rug Pull不同，Solana的统一SPL Token程序使欺诈行为转向链上操作（如市场操纵）。现有研究尚未对Solana上这些特定的Rug Pull模式进行系统性分析。

Method: 基于68个真实事件报告构建了包含117个确认Rug Pull代币的手动标注数据集，并分析了Solana上Rug Pull的工作流程。在此基础上提出了SolRugDetector检测系统，该系统仅使用链上交易和状态数据识别欺诈代币。在标注数据集上进行实验评估，并对2025年上半年新发行的100,063个代币进行大规模测量。
...
Conclusion: 该研究首次系统性地揭示了Solana上Rug Pull的生态系统特征，为理解Solana Rug Pull景观提供了见解，并支持开发有效的链上防御机制。研究发布了标注数据集和检测结果，有助于后续研究和实践应用。
...
### [75] [Bridging Code Property Graphs and Language Models for Program Analysis](https://arxiv.org/abs/2603.24837)
*Ahmed Lekssays*

Main category: cs.CR

TL;DR: CodeBadger是一个开源MCP服务器，将Joern的代码属性图引擎与LLM集成，解决了LLM分析真实代码库安全漏洞时的关键限制，支持跨函数和文件的程序切片、污点跟踪和数据流分析。


<details>
  <summary>Details</summary>
Motivation: LLM在分析真实代码库安全漏洞时面临三个关键挑战：1) token限制无法加载整个仓库；2) 代码嵌入无法捕获过程间数据流；3) LLM难以生成复杂的静态分析查询。现有方法只能处理孤立代码片段，无法发现跨多个函数和文件的漏洞。

Method: 开发CodeBadger开源MCP服务器，集成Joern的代码属性图引擎与LLM。不要求LLM生成复杂CPG查询，而是提供高级工具：程序切片、污点跟踪、数据流分析和语义代码导航，支持针对性探索大型代码库而无需详尽文件读取。

Result: 通过三个用例证明有效性：1) 在8,000个方法的代码库中导航审计内存安全模式；2) 发现并利用libtiff中先前未报告的缓冲区溢出漏洞；3) 首次尝试即为libxml2中的整数溢出漏洞(CVE-2025-6021)生成正确补丁。
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的MetaV方法需要大量多样化的模型集进行训练，这在现实场景中面临模型稀缺、训练成本高的问题。减少模型数量会导致过拟合，而MetaV的纠缠设计进一步加剧了这一问题，损害了对未见模型的验证泛化能力。

Method: 1. 检查点模型集增强策略：利用每个盗版模型和独立训练模型训练过程中的中间快照来丰富模型多样性，减少需要训练的大量模型数量。2. 本地验证器架构：为每个指纹配备轻量级本地验证器，减少参数纠缠并缓解过拟合问题。

Result: 在五个代表性任务上的广泛实验表明，LiteGuard在泛化性能和计算效率方面始终优于MetaV。

Conclusion: LiteGuard通过创新的检查点增强和本地验证器设计，实现了高效的任务无关模型指纹框架，在显著降低计算成本的同时获得了增强的泛化能力。
...
TL;DR: 提出IPK-pq：基于后量子密码的身份公钥管理框架，解决IIoT中传统PKI证书存储、验证和传输的扩展性问题，通过增强身份映射机制解决CPK的线性合谋问题，并在RPKI场景中验证了效率和可扩展性优势。


<details>
  <summary>Details</summary>
Motivation: 随着工业物联网的快速发展，传统基于证书的PKI在大规模证书存储、验证和频繁传输场景中的局限性日益凸显，后量子密码的广泛采用将进一步放大这些挑战，需要新的公钥管理方案。

Method: 提出IPK-pq身份公钥管理框架，基于NIST ML-DSA和随机矩阵理论实现身份密钥生成协议，在复合公钥概念基础上通过增强身份映射机制解决线性合谋问题，简化公钥真实性验证，降低证书管理复杂度，并提供形式化安全证明。

Result: 在典型PKI应用场景RPKI中实现并评估IPK-pq，对比实验结果显示基于IPK-pq的RPKI系统在效率和可扩展性方面有显著提升，验证了方案的可行性和合理性。

Conclusion: IPK-pq是下一代RPKI系统的有力候选方案，能够安全管理大规模路由信息，解决了传统PKI在后量子密码时代的扩展性挑战，为工业物联网等大规模场景提供了高效的身份公钥管理框架。
...
TL;DR: PIDP-Attack是一种针对RAG系统的复合攻击方法，通过结合提示注入和数据库投毒，无需预先知道用户查询即可操纵LLM响应，在多个基准数据集上显著优于传统投毒攻击。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统虽然能增强LLM的外部知识获取能力，但仍面临对抗攻击威胁。传统投毒攻击需要预先知道用户的具体查询，限制了其灵活性和实际应用性，因此需要开发更有效的攻击方法。

Method: 提出PIDP-Attack复合攻击策略：1) 在推理时向查询附加恶意字符实现提示注入；2) 向检索数据库中注入有限数量的投毒段落。这种方法无需预先知道用户实际查询，即可有效操纵LLM对任意查询的响应。

Result: 在三个基准数据集（Natural Questions、HotpotQA、MS-MARCO）和八个LLM上的实验评估表明，PIDP-Attack始终优于原始的PoisonedRAG。在开放域QA任务中，攻击成功率提高了4%到16%，同时保持高检索精度。

Conclusion: PIDP-Attack证明了复合攻击策略的必要性和高效性，揭示了RAG系统面临的新型安全威胁，为未来防御机制的设计提供了重要参考。
...
TL;DR: 该研究通过对照实验比较了Wi-Fi热点认证门户中Passkeys与密码的可用性，发现Passkeys在登录时感知可用性略优于密码但差异不显著，认证门户限制对用户体验有负面影响，平台特定问题带来可用性挑战。


<details>
  <summary>Details</summary>
Motivation: Passkeys作为新兴的无密码认证机制，在Wi-Fi热点认证门户中的可用性尚未被研究。认证门户作为公共Wi-Fi接入的关键环节，其认证体验对用户至关重要，需要实证研究来评估Passkeys在此场景下的表现。

Method: 采用对照实验室实验，50名参与者，采用裂区设计，覆盖Android和Windows平台，使用实现FIDO2CAP协议的路由器。通过定量和定性方法比较Passkeys与密码在认证门户中的可用性。

Result: 1. Passkeys在登录时感知可用性倾向于优于密码，但差异不显著；2. 无论认证方法如何，认证门户限制都负面影响用户体验并增加错误率；3. Passkeys在两个平台配置相对容易，但平台特定问题带来显著可用性挑战；4. 基于发现提出设计建议。

Conclusion: Passkeys在认证门户中具有潜力但面临平台特定挑战，认证门户本身的设计限制是主要问题。研究提出了改进认证门户认证的设计建议，包括无用户名认证流程、改进的认证门户检测机制和用户界面设计变更。
...
TL;DR: 该论文提出了一种针对基于深度学习的调制分类器的物理后门攻击方法，利用功率放大器非线性失真作为物理触发器，在训练数据形成前注入后门，攻击成功率高且能逃避现有防御技术。


<details>
  <summary>Details</summary>
Motivation: 深度学习在射频信号分类应用中已成为关键技术，但深度学习模型容易受到对抗性机器学习威胁。现有数字后门攻击在训练数据集中注入数字触发器，而本文研究物理后门攻击，利用功率放大器非线性失真作为物理触发器，在数据形成前注入后门，更具隐蔽性和实际威胁。

Method: 提出物理后门攻击方法：1）在训练阶段，攻击者操纵射频信号的幅度并改变其标签为目标调制方案，训练出后门模型；2）在推理阶段，攻击者保持后门攻击不活跃时，后门模型在测试信号上保持高准确率；3）当攻击者应用与训练阶段相同的操纵时，后门被激活，模型误分类这些信号。利用功率放大器非线性失真作为物理触发器，在数据集形成前创建物理触发器。

Result: 实验表明，所提出的攻击方法在不同噪声水平下，仅需少量操纵的射频信号即可实现高攻击成功率。此外，测试了该攻击对多种防御技术的抗性，结果显示这些防御技术无法有效缓解该攻击。

Conclusion: 该研究证明了物理后门攻击对基于深度学习的调制分类器的有效性，攻击成功率高且能逃避现有防御技术，揭示了深度学习模型在实际物理环境中的安全脆弱性，为射频信号处理系统的安全设计提供了重要警示。
...
TL;DR: 该论文研究了针对无线通信中自动调制分类的物理后门攻击，利用可解释AI指导触发器放置，并结合类原型与主成分生成触发器，在低信噪比和小毒化比例下实现高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 深度学习在无线通信自动调制分类中广泛应用，但易受对抗性机器学习威胁。现有研究多关注计算机视觉领域的威胁，缺乏针对无线通信物理层的后门攻击研究。

Method: 提出物理后门攻击方法：1) 使用可解释AI识别信号中最脆弱部分放置触发器；2) 结合类原型和主成分分析生成触发器；3) 在信号传输前注入物理后门。

Result: 攻击对多种基于深度学习的自动调制分类模型有效，在宽信噪比范围内和小毒化比例下均能实现高成功率。

Conclusion: 无线通信中的物理后门攻击具有高效性和隐蔽性，揭示了深度学习模型在无线通信应用中的安全脆弱性，需要开发相应的防御机制。
...
TL;DR: 提出MTCFuzz，一种针对操作系统与固件协同架构的多目标覆盖率引导灰盒模糊测试方法，相比单目标模糊测试能更深入探索系统


<details>
  <summary>Details</summary>
Motivation: 现代系统软件架构通常由固件和操作系统通过定义良好的接口协同工作，但传统模糊测试主要针对单一软件组件，忽视了软件间的协同执行

Method: 采用QEMU作为虚拟化环境，在虚拟机内启动系统进行模糊测试，统一观测操作系统和固件的执行，测量跨软件边界的代码覆盖率作为反馈

Result: 实现了名为MTCFuzz的多目标覆盖率引导灰盒模糊测试器，并评估了其有效性

Conclusion: 提出的多目标模糊测试方法能够利用协同软件组件的代码覆盖率作为反馈，相比传统单目标模糊测试实现更深入的系统探索
...
TL;DR: ALPS是一个自动化框架，通过静态分析和LLM生成安全策略，在无服务器环境中实施最小权限原则，支持多云平台和多种编程语言。
...
Method: ALPS采用针对无服务器环境的静态分析从函数代码中提取精确权限需求，使用微调的大型语言模型生成语言和供应商特定的安全策略，并进行实时监控以阻止未经授权的访问并适应策略或代码变化。
...
Conclusion: ALPS为无服务器工作负载提供了一个有效、实用且供应商无关的安全解决方案，能够显著改善无服务器环境中的权限管理安全性。
...
<details>
  <summary>Details</summary>
Motivation: 设备端视觉语言模型(VLMs)虽然通过本地执行保证了数据隐私，但动态高分辨率预处理(如AnyRes)的架构转变引入了固有的算法侧信道漏洞。与静态模型不同，动态预处理根据图像宽高比将图像分解为可变数量的补丁，创建了与工作负载相关的输入，这为攻击者提供了可观测的信号。

Method: 提出双层攻击框架：第一层，无特权攻击者利用标准操作系统指标中的显著执行时间变化，可靠地指纹输入图像的几何形状；第二层，通过分析末级缓存(LLC)争用，攻击者可以解析相同几何形状内的语义模糊性，区分视觉密集内容(如医学X光片)和稀疏内容(如文本文档)。评估了LLaVA-NeXT和Qwen2-VL等最先进模型。

Result: 结合执行时间和缓存争用信号，攻击者能够可靠地推断隐私敏感上下文。分析安全工程权衡发现，使用恒定工作填充的缓解方法会产生显著的性能开销。

Conclusion: 动态预处理在视觉语言模型中引入了可被利用的侧信道漏洞，威胁设备端AI的隐私保证。需要提出实用的设计建议来确保边缘AI部署的安全性，同时平衡性能与安全需求。
...
### [85] [Unveiling the Resilience of LLM-Enhanced Search Engines against Black-Hat SEO Manipulation](https://arxiv.org/abs/2603.25500)
*Pei Chen,Geng Hong,Xinyi Wu,Mengying Wu,Zixuan Zhu,Mingxuan Liu,Baojun Liu,Mi Zhang,Min Yang*

Main category: cs.CR

TL;DR: 首次系统研究针对大语言模型增强搜索引擎的SEO攻击，发现LLMSEs能抵御99.78%传统SEO攻击，但对新型LLMSEO攻击脆弱，改写查询和分段文本可使操纵率翻倍。


<details>
  <summary>Details</summary>
Motivation: 大语言模型增强搜索引擎在信息检索方面取得革命性进展，但其对黑帽SEO攻击的安全影响尚未被探索。本研究旨在填补这一空白，首次系统评估LLMSEs在SEO攻击下的安全性。

Method: 1) 选取10个代表性LLMSE产品（如ChatGPT、Gemini）；2) 构建SEO-Bench基准，包含1000个真实世界黑帽SEO网站；3) 评估开源和闭源LLMSEs；4) 提出并评估7种LLMSEO攻击策略。

Result: 1) LLMSEs能缓解99.78%的传统SEO攻击，检索阶段是主要过滤机制；2) 现成的LLMSEs对LLMSEO攻击脆弱，改写查询填充和分段文本使操纵率相比基线翻倍；3) 已向主要厂商负责任地报告发现的问题。

Conclusion: 本研究首次深入分析LLMSE生态系统的安全性，揭示了LLMSEs对新型LLMSEO攻击的脆弱性，为构建更具韧性的AI驱动搜索系统提供了实践见解。
...
### [86] [TAAC: A gate into Trustable Audio Affective Computing](https://arxiv.org/abs/2603.25570)
...
<details>
  <summary>Details</summary>
Motivation: 音频数据包含用户敏感身份信息，在智能诊断过程中易被恶意利用，现有方法难以清晰分离抑郁特征与敏感特征，需要安全加密方法和强大分类器

Method: 基于对抗损失子空间分解的TAAC框架，包含特征子空间分解器(DFSD)、灵活噪声加密器(FNE)和分阶段训练范式，分别用于特征分解、身份加密和性能增强

Result: 实验表明该框架在抑郁症检测、身份信息保留和音频重建方面优于现有加密方法，在不同加密强度下保持稳定性，具有优异的保密性、准确性、可追溯性和可调节性

Conclusion: TAAC框架成功解决了音频抑郁症诊断中的隐私保护问题，实现了在可信环境下的自动化抑郁症检测，平衡了诊断准确性和用户隐私保护
...
### [87] [AI Security in the Foundation Model Era: A Comprehensive Survey from a Unified Perspective](https://arxiv.org/abs/2603.24857)
*Zhenyi Wang,Siyu Luan*

Main category: cs.CR

TL;DR: 提出统一闭环威胁分类法，从四个方向轴分析机器学习中数据与模型的双向安全威胁


<details>
  <summary>Details</summary>
Motivation: 现有研究孤立看待机器学习安全威胁，缺乏揭示其共同原理和相互依赖性的统一框架，阻碍系统性理解和全面防御设计

Method: 提出统一闭环威胁分类法，从四个方向轴明确构建模型-数据交互框架：D→D、D→M、M→D、M→M

Result: 建立了四类相互关联的安全威胁类别，为分析基础模型安全提供了原则性视角

Conclusion: 该统一框架阐明了安全威胁之间的内在联系，为开发可扩展、可迁移和跨模态的安全策略奠定了基础
